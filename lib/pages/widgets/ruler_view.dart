...
class RulerState extends State<RulerView> {
  ...
  void initState() {
    ...
    _scrollController = ScrollController(
      ...
      (widget.currValue - widget.minValue) / widget.step * widget.subScaleWidth,
    );
  }
...
if(widget.isNeedBuild){
      ...
      child: Stack(
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: Image(image: (nimg_line), fit: BoxFit.fill,),
          ),
        ],
      ),
    );
  ...
  bool _onNotification(Notification notification) {
    ...
    if (notification is ScrollNotification) {
      ...
      if(centerValue<=5){
        centerValue=0;
      }else if(centerValue>=(widget.maxValue-5)){
        centerValue=widget.maxValue;
      }else {
        var lastIndex=5;
        for (var index=1;index <(widget.maxValue/10);index++ ){
          if(centerValue>=lastIndex&&centerValue<=lastIndex+10){
            centerValue=index*10;
          }
      ...
      if (_scrollingStopped(notification, _scrollController!)&&!widget.isNeedBuild) {
        select(centerValue);
      }else{
        widget.isNeedBuild=false;
      }
  ...
  void select(int centerValue) {
    ...
    // print("修正  centerValue $centerValue  widget.minValue  ${widget.minValue }  widget.step  ${widget.step}   ${widget.subScaleWidth}");
  ...
  ///选中值
  void jumpTo(int centerValue) {
    //根据（中间值-最小值）/步长=第几个刻度，然后第几个刻度乘以每个刻度的宽度就是移动的宽度
    double x =
        (centerValue - widget.minValue) / widget.step * widget.subScaleWidth;
    // print("修正  centerValue $centerValue  widget.minValue  ${widget.minValue }  widget.step  ${widget.step}   ${widget.subScaleWidth}");
    // _scrollController!.animateTo(x,
    _scrollController!.jumpTo(x);
  }
  ...
  void drawLine(Canvas canvas, Size size) {
    ...
    for (double x = 0; x <= size.width; x += subScaleWidth) {
      if (index % subScaleCountPerScale == 0) {
        canvas.drawLine(
            Offset(x, size.height-0), Offset(x, size.height-size.height / 3),  linePaint!);
      } else {
        canvas.drawLine(Offset(x, size.height-0 ), Offset(x,size.height-size.height / 5 ), linePaint!);
      }
      index++;
    }
  }
